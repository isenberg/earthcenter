...
// the line above makes this file executable as a script, but it can also be just started as explained below
/*
 * Geographic Center of Earth Calculator
 *
 * Calculates the geographic center of all land surfaces on Earth.
 *
 * This is the slow single-threaded variant EarthCenter2025SingleThread.java
 * of the faster parallelized EarthCenter2025.java and only added
 * for better code-readability and not intended for real use.
 *
...
public class EarthCenter2025SingleThread extends JPanel {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("version: slow single-threaded sequential gradient descent " + serialVersionUID);
	...
	static class GradientCalc {
		...
		public void calculate(Raster tiffDEM, int seaLevel, double startLat, double starLon, double startStep, int mapWidth, int calcWidth, String outputprefix, int antarcticLevel, boolean squareMode) throws IOException {
			...
			DoubleAdder landCountAdder = new DoubleAdder();
			DoubleAdder seaCountAdder = new DoubleAdder();
			...
			double elevationMeanGlobe = elevationSum / (landCount + seaCount);
			double elevationMeanLand = elevationSum / landCount;
		...
		MPoint nextPoint(MPoint currPoint, int mapStep, int step) {
			...
			MPoint next;
			for(int i=0; i < gradtest.length; i++) {
